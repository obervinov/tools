...
def get_cache_aliases_prod():
    ...
    for item in response:
        ...
        if (
            item_alias not in json_aliases
            and item["alias"] not in exclude_aliases_prod
            and not bool(re.match(r'\.', item["alias"]))
        ):
...
def get_cache_indices_prod():
    ...
    for item in response:
        ...
        if (
            item_index not in json_indices
            and item["index"] not in exclude_indices_prod
            and not bool(re.match(r'\.', item["index"]))
        ):
            json_indices.append(item_index)
    lld_json_indices = json.dumps({"data": json_indices})
